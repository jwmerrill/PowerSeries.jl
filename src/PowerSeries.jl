...
_diff{T<:Real}(p::PowerSeriesType{T}, n::T) = PowerSeriesType(n*p.re, _diff(p.ep, n + one(T)))

pint{T<:Real}(p::T) = PowerSeriesType(zero(T), p)
pint{T<:Real}(p::PowerSeriesType{T}) = PowerSeriesType(zero(T), _pint(p.re, p.ep, one(T)))

_pint{T<:Real}(r::T, e::T, n::T) = PowerSeriesType(r/n, e/(n + one(T)))
_pint{T<:Real}(r::T, e::PowerSeriesType{T}, n::T) =
